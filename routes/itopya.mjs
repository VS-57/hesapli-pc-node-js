...
async function parseTotalPages(dom) {
  const pageInfo = dom.window.document.querySelector(".page-info strong").textContent.trim();
  const totalPages = parseInt(pageInfo.split("/")[1], 10);
  return totalPages;
}

async function parseProducts(dom) {
  const productElements = dom.window.document.querySelectorAll(".product");
...
async function fetchAllProducts() {
  const dom = await fetchPageData(1);
  const totalPages = await parseTotalPages(dom);
  let allProducts = await parseProducts(dom);

  for (let page = 2; page <= totalPages; page++) {
    const dom = await fetchPageData(page);
    const products = await parseProducts(dom);
